...
import { ADJUST_LABEL, APPROVE_LABEL, REJECT_LABEL, RESULT_CARD_TOOL, REVIEW_QUESTION_ID, formatReviewDetail, resolveReview, } from './contract.js';
...
export function apply(ctx) {
    ...
    ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            if (exec.agent === undefined)
                throw new Error(`${RESULT_CARD_TOOL} requires a live calling agent`);
    ...
    const disableForSubagent = (agent) => {
        if (!isSubagentSession(agent.session.header))
            return;
